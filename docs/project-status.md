...
Current task count: 63 of N (Tasks 1–28.6 + Task 34–43 + Task W090–W109A complete)
Active branch: `fix/w109a-rtt-renegotiation-and-rtp-port-collision`
...
| SipCall | COMPLETE | 9-state SM, stub + PJSIP paths, audio/video/RTT signals. Task W109A split `acceptIncomingRttRequest()`/`rejectIncomingRttRequest()` from `requestRtt()` (single-flight guard + own-offer-completion detection in `onCallMediaState`) |
